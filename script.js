...
function pay(method){
  ...
  if(!seller) return alert("Selecciona el vendedor");
  ...
  const methodText = method === "paypal" ? "PayPal" : "Otro método de pago";
  ...
  // WhatsApp
  window.open(
    `https://wa.me/${s.phone}?text=${encodeURIComponent(msg)}`,
    "_blank"
  );
  ...
  if(method === "paypal"){
    setTimeout(() => {
      window.open(
        `https://www.paypal.com/cgi-bin/webscr?cmd=_xclick&business=${PAYPAL_EMAIL}&item_name=DZSTORE Pedido ${id}&amount=${total}&currency_code=USD`,
        "_blank"
      );
    }, 1200);
  }
